...
# upload and display ISEAR DATA:

my_data = pd.read_csv('project datasets/ISEAR_dataset.csv')
my_data = my_data.iloc[:, 0:2]

print('5 top examples:')
...
# check and fix spelling mistakes in the classes:

print(np.unique(my_data['emotion']))
my_data = my_data.replace('guit', 'guilt')
print(np.unique(my_data['emotion']))
...
# drop duplicates and display data again:
...
my_data = my_data[my_data.text != '[ Depressions.]']
my_data = my_data[my_data.text != 'NO RESPONSE.']
...
frequencies = dict(zip(ngrams, counts))
wordcloud = wordcloud.WordCloud(prefer_horizontal=0.8, background_color="white", mask=None).generate_from_frequencies(frequencies)
...
# # Present tf-idf Example:

# In[13]:


print(emotions_dict)
short_texts = my_data[(my_data.emotion==4)&(len_texts < 7)]
texts = [i for i in short_texts['text'].iloc[25:30]]
vectorizer = TfidfVectorizer(TfidfVectorizer(ngram_range=(1,2),
               min_df=0.1, max_df=0.9, strip_accents='unicode', use_idf=1,
               smooth_idf=1, sublinear_tf=1 ))
text_tfidf_matrix = vectorizer.fit_transform(texts)
vocabulary = vectorizer.vocabulary_
sorted_vocabulary = dict(sorted(vocabulary.items(), key=lambda x: x[1]))
display(sorted_vocabulary)

pandas_txt_matrix = pd.DataFrame(text_tfidf_matrix.todense(), index=texts)
pd.options.display.float_format = '{:,.2f}'.format
display(pandas_txt_matrix)
...
# Function for vectorizing texts into numerical values:
...
x_train, x_val, vectorizer, selector = ngram_vectorize(
...
# Saving vectorizer and selector objects for testing:
with open('saved models\ISEAR_vectorizer_N_selector.pkl', 'wb') as f: 
...
def train_ngram_model(x_train,
    ...
    # Save model.
    model.save('saved models\ISEAR_MLP_Model.h5')
...
fscore = np.expand_dims(fscore, axis=0)
...
cm = np.concatenate((cm, fscore), axis=0)
...
plt.yticks(list(range(len(emotions)+2)), np.append(emotions,['precision', 'fscore']))
plt.grid(False)
plt.show()
